...
Agent 使用 LangGraph SQLite checkpointer 按 `threadId` 保存会话历史。数据库位于当前工作目录 `.data/checkpointer.db`；每次 CLI 启动会创建新的会话 ID，因此不会自动引用上一次启动的对话。聊天过程中输入 `/new` 也会立即创建新的会话 ID，后续消息不会携带当前会话的历史。`.data/` 已被 Git 忽略。
...
聊天过程中可以输入 `/new` 开启新会话。交互命令会在本地解析，不会发送给 AI；未来可继续增加带参数的命令。
