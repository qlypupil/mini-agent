...
每轮 AI 成功回复后，终端会显示最终模型请求的 context token、该模型已知的上下文上限和占比。占比达到 80% 时，CLI 会立即调用独立的模型请求压缩历史 Context：至少保留最近 6 条原始消息，并将更早且尚未压缩的消息合并为累计摘要。摘要从下一轮对话开始生效；压缩失败不会影响已经完成的 AI 回复，下一轮再次达到阈值时会重试。若同一会话累计压缩达到 3 次，CLI 会继续压缩，同时强烈建议输入 `/new` 开启新会话。若兼容网关未返回流式 usage，或模型上限未被内置识别，对应字段会显示“未知”，不会使用不可靠的估算值，也不会触发自动压缩。
